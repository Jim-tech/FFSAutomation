...
def TEST_UGS():
    try:
        ...
        if ui_click_id_with_text(d, "mosaic.base_text", "Don't Have A Code?", "mosaic.pages.InstructionalPage-title", 10) is not True:
            logger.error("Unable to see the InstructionalPage(pairing mode prompt). Please make sure you have logged in Alexa App and the network is in good condition.")
            return False

        logger.info("clicking 'Next' to continue ...")
        if ui_click(d, "mosaic.pages.InstructionalPage-footer-primary-btn", "mosaic.pages.ConfirmationPage-title", 10) is not True:
            logger.error("Unable to see the ConfirmationPage. Please make sure you have logged in Alexa App and the network is in good condition.")
            return False
        ...
        time.sleep(2)
        d(resourceId="Mosaic.radio_list_item-primary", text=SAVED_WIFI_SSID).click()
...
if __name__ == "__main__":
    ...
    for i in range(MAXIMUM_TEST_COUNT):
        logger.info(f"=================== UGS test {i+1}/{MAXIMUM_TEST_COUNT} ===================")
        if TEST_UGS() == True:
        ...
        logger.info(f" Total executed UGS times: {success_cnt + failure_cnt} / {MAXIMUM_TEST_COUNT}")
        logger.info(f" Total successful UGS: {success_cnt}")
        logger.info(f" Total failed UGS: {failure_cnt}")
        logger.info(f"=======================================================")